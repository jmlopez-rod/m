...
The next sections will show how set up and use the `m-flow`. To follow along
use the [example-library] template, which is a basic node library that provides
one function.
...
At this point we may commit the changes and move on to create the scripts to
fun local builds.
...
Before we can run the script we need to make sure it has proper permissions
with `chmod +x m/publish.sh`. Now we are ready to run `make ciChecks`.
...
Note that on the `.npmrc` file we need to use your username or the name of
the organization. Commit the files and open up a pull request.
...
request and branches. If the pull request was successful we should now
be able to install the package in another project.
...
generate the build. At this point the pull request should be merged so that
we may start creating the builds in the `master` branch.
